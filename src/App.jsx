...
function Title({title}){
  return (
    <p className='font-TW text-(--Mcream) text-[40px] text-nowrap'>{title}</p>
  );
}
...
function SetActivePageItem({name}){
  ...
  return (
    <button className={`w-75 h-15 font-SA text-[35px] text-center tracking-wide leading-17 pl-2 font-light ${activeMenu === name? 'text-(--Mcream) bg-(--Mgreen) bg-opacity-100 shadow-[inset_0_0_15px_0.5px_rgba(255,255,255,0.3)]': 'text-(--Mgreen)'} hover:text-(--Mcream) hover:bg-(--Mgreen) hover:opacity-50 hover:shadow-[inset_0_0_15px_0.5px_rgba(255,255,255,0.3)] cursor-pointer`} onClick={()=>setActiveMenu(name)}>{name}</button>
  );
}

function Navigation(){
  return(
    <nav className='nav ml-[80%] transition-all duration-350 ease-in-out'>
            <div>
            <div>
              ...
              <div className='MenuList transition-all duration-400 delay-100 opacity-0 h-dvh w-75 bg-(--Mcream) flex flex-col items-center justify-start absolute -top-10 right-10 rounded-bl-[20px] rounded-tl-[20px]'>
